...
\textbf{Uncertainty Shocks.} Innovations to the forecast error variance of aggregate variables orthogonal to any other known economic disturbance.
$$
...
\textbf{Cash and Cash Equivalents} refer to assets a business hold as ready cash
\begin{itemize}
...
\frame{\frametitle{Appendix A - Simulated Data}
	...
	\begin{eqnarray*}
	\begin{cases}
	X_t = \rho_{XX} X_{t-1} + \rho_{XY} Y_{t-1}  + \rho_{XZ} Z_{t-1}  +  \alpha_{1X}s_t^1  +  \alpha_{2X}s_t^2 \\
	Y_t = \rho_{YX} X_{t-1} + \rho_{YY} Y_{t-1}  + \rho_{YZ} Z_{t-1}  +  \alpha_{1Y}s_t^1  +  \alpha_{2Y}s_t^2 \\
	Z_t = \rho_{ZX} X_{t-1} - \rho_{ZY} Y_{t-1}  + \rho_{ZZ} Z_{t-1}  +  \alpha_{1Z}s_t^1  -  \alpha_{2Z}s_t^2
	\end{cases}
	\end{eqnarray*}
	
	Objective is to estimate $s_t^1$ and $s_t^2$ under the assumption that we only know the sign of $\alpha_{ij}$, $i=\{1,2\}$, and $j = \{X, Y, Z\}$.
